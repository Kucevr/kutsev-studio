      ...
      <div className="container mx-auto px-4 md:px-6">
        <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-8">
          <div className="overflow-hidden max-w-full">
            <h2 className="text-fluid-huge font-display font-bold tracking-tighter leading-[0.85] uppercase wrap-break-word">
              <TextReveal>{t('showcase.title')}</TextReveal>
            </h2>
          </div>
          <p className="text-xl md:text-2xl font-medium max-w-sm text-left md:text-right">
            {t('showcase.subtitle')}
          </p>
        </div>
        ...
        <div className="flex flex-col gap-24">
          {showcaseProjects.map((project, index) => (
            <div 
              key={project.id} 
              className={`group relative flex flex-col md:flex-row gap-12 items-center cursor-pointer ${index % 2 === 1 ? 'md:flex-row-reverse' : ''}`}
              onClick={() => handleProjectClick(project)}
              data-cursor-text="VIEW"
            >
              <div className="w-full md:w-3/5 relative overflow-hidden rounded-lg aspect-[16/9] shadow-lg">
                ...
                <div className="absolute inset-0 z-20 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none">
                   <div className="w-32 h-32 bg-white/20 backdrop-blur-md rounded-full flex items-center justify-center border border-white/30 text-white font-bold tracking-widest uppercase text-sm shadow-2xl">
                      {t('showcase.open')}
                   </div>
                </div>
              </div>
              ...
              <div className="w-full md:w-2/5 flex flex-col justify-center">
                <span className="text-xs font-bold border border-black/20 self-start px-3 py-1 rounded-full mb-4">
                  0{index + 1}
                </span>
                ...
                <p className="text-lg text-gray-500 font-medium mb-6">{project.category}</p>
                <button className="flex items-center gap-2 text-sm font-bold uppercase tracking-wider group-hover:gap-4 transition-all link-underline w-fit">
