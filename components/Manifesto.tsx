...
const SplitText = memo(({ text, progress }: { text: string, progress: number }) => {
          ...
          <span key={i} className="relative inline-block">
            <span 
              ...
              style={{
                ...
                filter: charProgress > 0.9 ? 'none' : `blur(${(1 - charProgress) * 4}px)`,
                transitionProperty: charProgress > 0 && charProgress < 1 ? 'all' : 'none'
              }}
            >
              {char === ' ' ? '\u00A0' : char}
            </span>
            ...
            {charProgress > 0.1 && charProgress < 0.9 && (
              <span 
                className="absolute left-0 right-0 h-0.5 bg-brand-accent/50 z-20 blur-[2px]"
...
const PrincipleItem = memo(({
  ...
  const words = useMemo(() => item.desc.split(' '), [item.desc]);
        ...
        <div className="text-2xl md:text-5xl font-light leading-[1.05] tracking-tight relative text-gray-500">
          <p className="relative z-10">
            {words.map((word: string, i: number) => {
              const wordStart = i / words.length * 0.2;
              ...
              return (
                <span 
                  ...
                  style={{ 
                    ...
                    transitionProperty: wordProgress > 0 && wordProgress < 1 ? 'all' : 'none'
                  }}
                >
                  {word}
                </span>
...
export const Manifesto: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const handleScroll = throttle(() => {
      ...
      // Track active principle (less frequent)
    ...
    >
      ...
      <div className="hidden md:block absolute top-0 left-0 w-full h-full pointer-events-none opacity-20">
        ...
        <div 
          ...
          style={{ transform: `translateY(${-scrollProgress * 150}px) scale(${1.5 - scrollProgress * 0.5})` }}
        />
      </div>
